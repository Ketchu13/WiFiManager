...
bool WiFiManager::startAP() {
  ...
  DEBUG_WM(F("AP MAC address:"), WiFi.macAddress());
#endif
...
boolean WiFiManager::startConfigPortal(char const *apName, char const *apPassword) {
  ...
  // check if appName is valid for ssid
  // authorised chars are 0x20, 0x21, 0x23, 0x25-0x2A, 0x2C-0x3E, 0x40-0x5a, 0x5e-0x7e at any position
  ...
  for (unsigned int i = 0; i < strlen(_apName.c_str()); i++) {
    if (i != 0 && i!=(strlen(_apName.c_str())-1)) {
      ...
      // check if valid char
      if (_apName.c_str()[i] == 0x20 ||
          _apName.c_str()[i] == 0x21  ||
          _apName.c_str()[i] == 0x23  ||
          (_apName.c_str()[i] >= 0x25 && _apName.c_str()[i] <= 0x2A) ||
          (_apName.c_str()[i] >= 0x2C && _apName.c_str()[i] <= 0x3E) ||
          (_apName.c_str()[i] >= 0x40 && _apName.c_str()[i] <= 0x5A) || 
          (_apName.c_str()[i] >= 0x5E && _apName.c_str()[i] < 0x7e)) {
        // valid char
        // do nothing
      } else  {
        // invalid char
      ...
      if(_apName.c_str()[i] == 0x20 || _apName.c_str()[i] == 0x2E) {
        _apName.remove(i);
      }
    }
  }
...
String WiFiManager::getDefaultAPName() {
  String hostString = WiFi.macAddress();
  hostString.replace(":", "");
  hostString = hostString.substring(hostString.length() - 4);
